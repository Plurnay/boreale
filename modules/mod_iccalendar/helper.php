...
//$this->date_start = JHtml::date($date_start, 'Y-m-01', null);
...
//									$d_date = date('Y-m-d', strtotime($d));
									$dy_date = JHtml::date($dy['date'], 'Y-m-d', $eventTimeZone);
//									$dy_date = date('Y-m-d', strtotime($dy['date']));
//echo '<br />' . $dy_date;

									if ($d_date == $dy_date)
									{
//echo '<br />' . $d_date . ' | ' . $dy_date;
...
//			$datejour = date('Y-m-d', strtotime($this_date_a));
			$datejour = JHtml::date($this_date_a, 'Y-m-d', null);
			$this_year_month = $year . '-' . $month . '-00';

			$list[$a]['datecal']	= JHtml::date($this_date_a, 'd', null);
			$list[$a]['monthcal']	= JHtml::date($this_date_a, 'm', null);
			$list[$a]['yearcal']	= JHtml::date($this_date_a, 'Y', null);

			$list[$a]['date']		= date('Y-m-d H:i', strtotime($this_date_a));
			$list_a_date		= date('Y-m-d H:i', strtotime($this_date_a));
//			$list[$a]['date']		= JHtml::date($this_year_month, 'Y-m', null) . '-' . $a;
...
//			$list[$a]['week']		= JHtml::date($this_date_a, 'N', null);

			$list[$a]['day']		= '<div class="' . $classDay . '">' . $a . '</div>';

			// Set cal_date
			$list[$a]['this_day']	= substr($list_a_date, 0, 10);
//			$list[$a]['this_day']	= JHtml::date(substr($list[$a]['date'], 0, 10), 'Y-m-d', null);

			// Added in 2.1.2 (change in NAME_day.php)
			$list[$a]['ifToday']	= $classDay;
//			$list[$a]['Days']		= $a;
			$list[$a]['Days']		= $list[$a]['datecal'];
			//
//			$list[$a]['month']		= $month;
//			$list[$a]['month']		= $list[$a]['monthcal'];
//			$list[$a]['year']		= $year;
//			$list[$a]['year']		= $list[$a]['yearcal'];
			$list[$a]['events']		= array();
...
//				array_push($da, JHtml::date($d, 'Y-m-d H:i', null));
			}
		}
...
//		$t_day		= JHtml::date($dateFormat, 'd', null);
		$t_month	= date('F', strtotime($dateFormat));
//		$t_year		= JHtml::date($dateFormat, 'Y', null);
