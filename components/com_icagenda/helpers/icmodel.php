...
//		$icu_layout = JRequest::getVar('layout', '');
	...
	protected function titleFormat($i)
	{
		...
		if ($text_transform == 1)
		{
			return ucfirst(strtolower($i->title));
		}
		elseif ($text_transform == 2)
		{
			return ucwords(strtolower($i->title));
		}
		elseif ($text_transform == 3)
		{
			return strtoupper($i->title);
		}
		elseif ($text_transform == 4)
		{
			return strtolower($i->title);
		}
		...
		foreach ($allDates as $k => $d)
		{
				...
				&& $nb_tickets_left > 0)
			{
				...
				else
				{
					...
					if ($this->displaytime($i) == 1)
					{
						$upDays[$k] = $datetime_date.'@@'.$date.' - '.date($lang_time, strtotime($datetime_date)) . $tickets_left;
					}
					else
					{
						$upDays[$k] = $datetime_date.'@@'.$date . $tickets_left;
					}
				}
			}
		}
...
//		$dateFormat = date('d-M-Y', strtotime($date));
		$dateFormat = JHtml::date($date, 'Y-m-d', $eventTimeZone);
		...
		foreach ($exformat as $k => $val)
		{
			switch($val)
			{
				...
				// year (v3)
				case 'Y':
//					$val = JHtml::date($dateFormat, 'Y', $eventTimeZone);
